...
NOTTY_VERSION="$(opam info notty -f installed-version 2>&1)"
if [[ "$NOTTY_VERSION" =~ "0.2.1" && "$NOTTY_VERSION" =~ "4.06.0" ]]; then
  echo "Notty version 0.2.1 is active. Good."
else
  echo "Notty version 0.2.1 is NOT active. This is bad."
  environment=bad.
fi
